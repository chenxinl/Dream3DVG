...
class SparseCoordInit:
    ...
    def __init__(self, pred, gt, format='[bs x c x 2D]', quantile_interval=200, nodiff_thres=0.1):
        ...
        # -> computes the actual quantile values at those thresholds for the error map (self.map), resulting in a series of threshold values (quantized_interval) 
        # -> that can be used to segment the error map into different levels of error severity
        ...
        # remove redundant
        quantized_interval = np.unique(quantized_interval) # remove unique elements in the quantized interval
        quantized_interval = sorted(quantized_interval[1:-1])
        self.map = np.digitize(self.map, quantized_interval, right=False) # get which bins of actual map locate in the quantized_interval
