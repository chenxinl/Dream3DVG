...
import numpy as np
from pytorch_svgrender.utils.graphics_utils import getWorld2View2, getProjectionMatrix, fov2focal, getWorld2View3, fov2focal
...
class RCamera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, uid, delta_polar, delta_azimuth, delta_radius, opt,
        ...
        self.intrinsic = (
                torch.Tensor([[self.focal_x, 0, self.image_width / 2], [0, self.focal_y, self.image_height / 2], [0, 0, 1]])
                .float()
                .cuda()
                )
