...
from pytorch_svgrender.painter.dream3dvg.loss.clip import CLIPVisualEncoder
from pytorch_svgrender.painter.dream3dvg.loss.lpips import LPIPS
from pytorch_svgrender.utils.misc import MODEL_TYPES, DIST_TYPES, get_augment_trans
from pytorch_svgrender.utils.math_utils import cos_distance
...
class CLIPLoss(nn.Module):
    ...
    def forward(
        ...
        for _ in range(self.num_augs):
            ...
            sketch_augs.append(augmented_pair.chunk(2)[0])
...
class CLIPConvLoss(nn.Module):
    ...
    def _L2(
        ...
        return [torch.square(f1.float() - f2.float()).mean() for f1, f2 in zip(features1, featrues2)]
    ...
    def _cosine(
        ...
        if RN:
            return [
                torch.square(f1.float(), f2.float(), dim=1).mean()
    ...
    def forward(
        ...
        if train:
            for _ in range(self.num_augs):
                ...
                sketch_augs.append(pair.chunk(2)[0])
                target_augs.append(pair.chunk(2)[1])
