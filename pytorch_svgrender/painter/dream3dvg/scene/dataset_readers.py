...
from pytorch_svgrender.utils.graphics_utils import getWorld2View2, focal2fov, fov2focal
import numpy as np
import json
from pathlib import Path
from pytorch_svgrender.utils.pointe_utils import init_from_pointe
from pytorch_svgrender.utils.sh_utils import SH2RGB
...
def readCircleCamInfo(path, opt):
    ...
    if not os.path.exists(ply_path) or opt.reinit_points:
        ...
        print("[INFO]: init from {}".format(opt.init_shape))
...
def gen_random_pos(size, param_range, gamma=1):
    ...
    rand_ = sign * (rand_ ** gamma)  # since all rand_ < 1, so gamma > 1 means all focus near mean value and gamma < 1 means far from mean value
...
def rand_poses(size, opt, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], angle_overhead=30, angle_front=60, uniform_sphere_rate=0.5, rand_cam_gamma=1):
    ...
    radius = gen_random_pos(size, radius_range) # return random radius in the range, size(size)
    ...
    # lookat
    forward_vector = safe_normalize(centers - targets) # z(camera), nerf coord, -z
    up_vector = torch.FloatTensor([0, 0, 1]).unsqueeze(0).repeat(size, 1) # y, may not perpendicular to z now | +Z (world)
    right_vector = safe_normalize(torch.cross(forward_vector, up_vector, dim=-1)) # x, get x perpendicular to y-z plane
    ...
    up_vector = safe_normalize(torch.cross(right_vector, forward_vector, dim=-1) + up_noise) #forward_vector, use z and x to recalculate the y-axis, which perpendicular to both x and z

    poses = torch.eye(4, dtype=torch.float).unsqueeze(0).repeat(size, 1, 1)
    poses[:, :3, :3] = torch.stack((-right_vector, up_vector, forward_vector), dim=-1) # [-x, y, z]
    poses[:, :3, 3] = centers
...
def GenerateRandomCameras(opt, size=2000, SSAA=True):
    ...
    delta_radius = radius - opt.default_radius
    # random focal
    fov = random.random() * (opt.fovy_range[1] - opt.fovy_range[0]) + opt.fovy_range[0] # Q: here only 1 fov, why not [size]?
