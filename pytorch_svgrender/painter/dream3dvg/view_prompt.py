...
from diffusers import StableDiffusionPipeline, DiffusionPipeline, DDPMScheduler, DDIMScheduler
import math

@torch.no_grad()
def get_text_embeds(diffusion, prompt, resolution=(512, 512)):
    inputs = diffusion.tokenizer(prompt, padding='max_length', max_length=diffusion.tokenizer.model_max_length, truncation=True, return_tensors='pt')
    embeddings = diffusion.text_encoder(inputs.input_ids.to(diffusion.device))[0]
    return embeddings
    
def get_text_embeddings(diffusion, prompt, neg_prompt):
    embeddings = {}
    embeddings['default'] = get_text_embeds(diffusion, [prompt])
    embeddings['uncond'] = get_text_embeds(diffusion, [neg_prompt])

    for d in ['front', 'side', 'back']: # TODO : why not add right size and left side ?
        embeddings[d] = get_text_embeds(diffusion, [f"{prompt}, {d} view"])
    embeddings['inverse'] = get_text_embeds(diffusion, [''])
    return embeddings

def interpolate_embeddings(embeddings, azimuth):
...
def adjust_text_embeddings(embeddings, azimuth):
    ...
    text_z_, weights_ = get_pos_neg_text_embeddings(embeddings, azimuth)
...
def weighted_perpendicular_aggregator(delta_noise_preds, weights, batch_size):
    ...
    """
    delta_noise_preds = delta_noise_preds.split(batch_size, dim=0) # K x [B, 4, 64, 64] <- Tuple
    weights = weights.split(batch_size, dim=0) # K x [B] <- Tuple
